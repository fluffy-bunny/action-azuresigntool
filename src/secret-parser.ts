...
export class SecretParser {
  ...
  constructor(content: string, contentType: FormatType) {
    switch (contentType) {
      ...
      default:
        throw new Error(
          `Given format: ${contentType} is not supported. Valid options are JSON, XML.`
        )
    }
  ...
  private extractJsonPath(
    ...
    const value = jp.query(this.dom, key)
    if (value.length === 0) {
      core.debug(`Cannot find key: ${key}`)
      return ''
    } else if (value.length > 1) {
      core.debug(
        `Multiple values found for key: ${key}. Please give jsonPath which points to a single value.`
      )
      return ''
    }
